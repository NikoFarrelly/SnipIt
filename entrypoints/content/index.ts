...
import {
  getSnipsForURL,
  resetCurrentPageSnipAmount,
  runSnipsOnLoad,
} from "@/src/snips.utils";
  ...
  main() {
    ...
    browser.runtime.sendMessage(
      ...
      async (response: {
        ...
        if (response.success && response.tab.active && response.tab.url) {
          ...
          const snipsOnSnip = snipsForURL.filter(
            (s) => !s.runOnPageLoad && s.currentPageSnipAmount > 0,
          );
    ...
    browser.runtime.onMessage.addListener(function (request, _, sendResponse) {
      if (request.action === "fireSnipOnClick") {
        if (!request?.fromText || !request?.untilClassName)
          return sendResponse({
            error: "No props given to remove elements.",
            success: false,
          });
        const { fromText, untilClassName } = request;
        const { removedElements, matchedElements } = removeElements({
          fromText,
          untilClassName,
        });
