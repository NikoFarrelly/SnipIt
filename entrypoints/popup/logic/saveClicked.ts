...
export const saveClicked = async () => {
  ...
  const runOnPageLoad = (
    document.getElementById("runOnPageLoad") as HTMLInputElement
  ).checked;
  const snippedAmount = document.getElementById(
    "addSnipAmount",
  ) as HTMLParagraphElement;
  const parsedAmount = snippedAmount?.innerText
    ? parseInt(snippedAmount.innerText)
    : 0;
