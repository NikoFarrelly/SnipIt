 ...
 */
const getRemovedElementsData = async (): Promise<{
  snipsAllTime: number;
  snipsOnThisPage: number;
} | null> => {
  ...
  const snipsOnThisPage = snipsForThisURL.reduce(
    (prev, curr) => prev + curr.currentPageSnipAmount,
    0,
  );
 ...
 */
export const updateAllTimeAndPageSnips = async (
  snippedElements: number,
): Promise<void> => {
 ...
 */
export const updateOnThisPageSnipsElement = (
  snipsOnThisPageAmount: number,
): void => {
  const amount = document.getElementById("snippedPageAmount");

  if (amount) {
    const amountNumber = isNaN(Number(amount.innerText))
      ? 0
