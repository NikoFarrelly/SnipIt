...
export const addSnipExpandedElement = async (
  givenSnip: Snip,
): Promise<void> => {
  ...
  updateButton.addEventListener("click", async () => {
    const fromText = (
      snipExpanded.querySelector("#fromText") as HTMLInputElement
    )?.value;
    const untilClassName = (
      snipExpanded.querySelector("#untilClass") as HTMLInputElement
    )?.value;
    const runOnPageLoad = (
      snipExpanded.querySelector("#runOnPageLoad") as HTMLInputElement
    )?.checked;
...
export const updateExpandedSnipAmount = (id: string, amount: string) => {
  const expandedCardSnipAmount = document.getElementById(
    id + "-expandedSnipAmount",
  );
